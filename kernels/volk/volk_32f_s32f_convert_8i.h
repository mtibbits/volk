...
static inline void volk_32f_s32f_convert_8i_u_avx(int8_t* outputVector,
                                                   const float* inputVector,
                                                   const float scalar,
                                                   unsigned int num_points)
{
    ...
    for (unsigned int number = 0; number < sixteenthPoints; number++) {
        ...
        // Extract 128-bit halves (AVX has no 256-bit integer pack)
        __m128i lo1 = _mm256_castsi256_si128(intInputVal1);
        __m128i hi1 = _mm256_extractf128_si256(intInputVal1, 1);
        __m128i lo2 = _mm256_castsi256_si128(intInputVal2);
        __m128i hi2 = _mm256_extractf128_si256(intInputVal2, 1);

        // Pack int32 -> int16 -> int8 using SSE2
        __m128i packed16_1 = _mm_packs_epi32(lo1, hi1);
        __m128i packed16_2 = _mm_packs_epi32(lo2, hi2);
        __m128i result = _mm_packs_epi16(packed16_1, packed16_2);

        _mm_storeu_si128((__m128i*)outputVectorPtr, result);
        outputVectorPtr += 16;
    }

    volk_32f_s32f_convert_8i_generic(outputVector + sixteenthPoints * 16,
...
static inline void volk_32f_s32f_convert_8i_u_avx2_fma(int8_t* outputVector,
                                                        const float* inputVector,
                                                        const float scalar,
                                                        unsigned int num_points)
{
    ...
    volk_32f_s32f_convert_8i_generic(outputVector + thirtysecondPoints * 32,
...
static inline void volk_32f_s32f_convert_8i_u_avx512bw(int8_t* outputVector,
                                                        const float* inputVector,
                                                        const float scalar,
                                                        unsigned int num_points)
{
    ...
    for (; number < sixtyfourthPoints; number++) {
        ...
        inputVal3 = _mm512_max_ps(
            _mm512_min_ps(_mm512_mul_ps(inputVal3, vScalar), vmax_val), vmin_val);
        inputVal4 = _mm512_max_ps(
            _mm512_min_ps(_mm512_mul_ps(inputVal4, vScalar), vmax_val), vmin_val);
        ...
        // Pack int16 -> int8 (with lane fix)
        __m512i packed8 =
            _mm512_permutexvar_epi64(fix_idx64, _mm512_packs_epi16(packed16_12, packed16_34));
...
extern void volk_32f_s32f_convert_8i_a_orc_impl(int8_t* outputVector,
                                                 const float* inputVector,
                                                 const float scalar,
                                                 int num_points);

static inline void volk_32f_s32f_convert_8i_u_orc(int8_t* outputVector,
                                                   const float* inputVector,
                                                   const float scalar,
                                                   unsigned int num_points)
{
    volk_32f_s32f_convert_8i_a_orc_impl(outputVector, inputVector, scalar, num_points);
}
...
static inline void volk_32f_s32f_convert_8i_a_avx(int8_t* outputVector,
                                                   const float* inputVector,
                                                   const float scalar,
                                                   unsigned int num_points)
{
    ...
    for (unsigned int number = 0; number < sixteenthPoints; number++) {
        ...
        _mm_store_si128((__m128i*)outputVectorPtr, result);
        outputVectorPtr += 16;
    }
...
static inline void volk_32f_s32f_convert_8i_a_avx2_fma(int8_t* outputVector,
                                                        const float* inputVector,
                                                        const float scalar,
                                                        unsigned int num_points)
{
...
static inline void volk_32f_s32f_convert_8i_a_avx512bw(int8_t* outputVector,
                                                        const float* inputVector,
                                                        const float scalar,
                                                        unsigned int num_points)
{
