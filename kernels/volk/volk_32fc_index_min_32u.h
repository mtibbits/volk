                                                            ...
                                                            uint32_t num_points)
{
    ...
    if (processed < num_points) {
        uint32_t tailIdx = 0;
        volk_32fc_index_min_32u_generic(&tailIdx, source, num_points - processed);
                                                     ...
                                                     uint32_t num_points)
{
    ...
    if (processed < num_points) {
        uint32_t tailIdx = 0;
        volk_32fc_index_min_32u_generic(&tailIdx, src0Ptr, num_points - processed);
                                                ...
                                                uint32_t num_points)
{
    ...
    if (num_points) {
        ...
        if (processed < num_points) {
            uint32_t tailIdx = 0;
            volk_32fc_index_min_32u_generic(&tailIdx, sourcePtr, num_points - processed);
            float tailVal =
                lv_creal(sourcePtr[tailIdx]) * lv_creal(sourcePtr[tailIdx]) +
                lv_cimag(sourcePtr[tailIdx]) * lv_cimag(sourcePtr[tailIdx]);
            if (tailVal < min) {
                min = tailVal;
                index = processed + tailIdx;
            }
                                                  ...
                                                  uint32_t num_points)
{
    ...
    if (processed < num_points) {
        uint32_t tailIdx = 0;
        volk_32fc_index_min_32u_generic(
