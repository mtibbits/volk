...
static inline void volk_32fc_index_max_32u_u_sse(uint32_t* target,
                                                  const lv_32fc_t* src0,
                                                  uint32_t num_points)
{
    ...
    for (unsigned i = 0; i < num_points / 4u; ++i) {
        ...
        /* Swap adjacent elements: [re², im², ...] → [im², re², ...] */
        ...
        /* Pick re²+im² lanes: [mag0, mag1, mag2, mag3] */
    ...
    /* Handle tail */
    const uint32_t tail_start = num_points & (~3u);
    const uint32_t tail_count = num_points - tail_start;
    if (tail_count > 0) {
        uint32_t tail_index;
        volk_32fc_index_max_32u_generic(&tail_index, src0, tail_count);
...
static inline void volk_32fc_index_max_32u_u_sse3(uint32_t* target,
                                                   const lv_32fc_t* src0,
                                                   uint32_t num_points)
{
...
static inline void volk_32fc_index_max_32u_u_avx(uint32_t* target,
                                                   const lv_32fc_t* src0,
                                                   uint32_t num_points)
{
    ...
    /* Handle tail */
    const uint32_t tail_start = num_points & (~7u);
    const uint32_t tail_count = num_points - tail_start;
    if (tail_count > 0) {
        uint32_t tail_index;
        volk_32fc_index_max_32u_generic(&tail_index, src0, tail_count);
...
static inline void volk_32fc_index_max_32u_a_sse(uint32_t* target,
                                                  const lv_32fc_t* src0,
                                                  uint32_t num_points)
{
    const __m128 indices_increment = _mm_set1_ps(4.0f);
...
static inline void volk_32fc_index_max_32u_a_avx(uint32_t* target,
                                                   const lv_32fc_t* src0,
                                                   uint32_t num_points)
{
    const __m256 indices_increment = _mm256_set1_ps(8.0f);
