 ...
 * \b Overview
 *
 * Computes the dot product (inner product) of a complex input vector and a
 * real-valued tap vector: result = sum(input[i] * taps[i]). Each real tap
 * scales both the real and imaginary parts of the corresponding complex sample.
 *
 * This kernel is commonly used in FIR filtering where the input samples are
 * complex (e.g. baseband I/Q data) and the filter coefficients (taps) are
 * real-valued. It also appears in matched filtering, correlation, and
 * beamforming operations.
 *
 * <b>Dispatcher Prototype</b>
 * \code
 * void volk_32fc_32f_dot_prod_32fc(lv_32fc_t* result, const lv_32fc_t* input,
 *   const float* taps, unsigned int num_points)
 * \endcode
 *
 * \b Inputs
 * \li input: The complex input samples (lv_32fc_t).
 * \li taps: The real-valued filter taps (float).
 ...
 *   input[i] = lv_cmake(3.0f, -1.0f);
 *   taps[i] = 2.0f;
 * }
 *
 * // Expected: 4 * (3.0 - 1.0j) * 2.0 = 24.0 - 8.0j
 * lv_32fc_t expected = lv_cmake(24.0f, -8.0f);
 *
 * volk_32fc_32f_dot_prod_32fc(&result, input, taps, N);
 *
...
static inline void volk_32fc_32f_dot_prod_32fc_u_sse3(lv_32fc_t* result,
                                                       const lv_32fc_t* input,
                                                       const float* taps,
                                                       unsigned int num_points)
{
    ...
    for (; number < eighthPoints; number++) {
        ...
        /* Single load + 2 shuffles per 4 taps (vs SSE's 2 loads + 2 unpacks) */
        x0Val = _mm_loadu_ps(bPtr);
        x1Val = _mm_loadu_ps(bPtr + 4);
        b0Val = _mm_shuffle_ps(x0Val, x0Val, _MM_SHUFFLE(1, 1, 0, 0));
        b1Val = _mm_shuffle_ps(x0Val, x0Val, _MM_SHUFFLE(3, 3, 2, 2));
        b2Val = _mm_shuffle_ps(x1Val, x1Val, _MM_SHUFFLE(1, 1, 0, 0));
        b3Val = _mm_shuffle_ps(x1Val, x1Val, _MM_SHUFFLE(3, 3, 2, 2));
...
static inline void volk_32fc_32f_dot_prod_32fc_u_avx_fma(lv_32fc_t* result,
                                                          const lv_32fc_t* input,
                                                          const float* taps,
                                                          unsigned int num_points)
{
    ...
    for (; number < sixteenthPoints; number++) {
        ...
        x0Val = _mm256_loadu_ps(bPtr);
        x1Val = _mm256_loadu_ps(bPtr + 8);
        x0loVal = _mm256_unpacklo_ps(x0Val, x0Val);
        x0hiVal = _mm256_unpackhi_ps(x0Val, x0Val);
        x1loVal = _mm256_unpacklo_ps(x1Val, x1Val);
        x1hiVal = _mm256_unpackhi_ps(x1Val, x1Val);
...
static inline void volk_32fc_32f_dot_prod_32fc_a_sse3(lv_32fc_t* result,
                                                       const lv_32fc_t* input,
                                                       const float* taps,
                                                       unsigned int num_points)
{
...
static inline void volk_32fc_32f_dot_prod_32fc_a_avx_fma(lv_32fc_t* result,
                                                          const lv_32fc_t* input,
                                                          const float* taps,
                                                          unsigned int num_points)
{
...
extern void volk_32fc_32f_dot_prod_32fc_neonasm(lv_32fc_t* result,
                                                const lv_32fc_t* input,
                                                const float* taps,
                                                unsigned int num_points);
...
#ifdef LV_HAVE_NEONV7
extern void volk_32fc_32f_dot_prod_32fc_neonasmvmla(lv_32fc_t* result,
                                                    const lv_32fc_t* input,
                                                    const float* taps,
                                                    unsigned int num_points);
#endif /*LV_HAVE_NEONV7*/
...
#ifdef LV_HAVE_NEONV7
extern void volk_32fc_32f_dot_prod_32fc_neonpipeline(lv_32fc_t* result,
                                                     const lv_32fc_t* input,
                                                     const float* taps,
                                                     unsigned int num_points);
#endif /*LV_HAVE_NEONV7*/
