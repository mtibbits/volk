...
static inline void volk_32fc_index_max_16u_u_sse(uint16_t* target,
                                                  const lv_32fc_t* src0,
                                                  uint32_t num_points)
{
    num_points = (num_points > USHRT_MAX) ? USHRT_MAX : num_points;

    const __m128 indices_increment = _mm_set1_ps(4.0f);
    __m128 current_indices = _mm_setr_ps(0.0f, 1.0f, 2.0f, 3.0f);

    __m128 max_values = _mm_setzero_ps();
    __m128 max_indices = _mm_setzero_ps();

    for (unsigned i = 0; i < num_points / 4u; ++i) {
        __m128 in0 = _mm_loadu_ps((const float*)src0);
        __m128 in1 = _mm_loadu_ps((const float*)(src0 + 2));
        src0 += 4;
        ...
        __m128 cmp_gt = _mm_cmpgt_ps(mag_sq, max_values);
        max_indices = _mm_or_ps(_mm_and_ps(cmp_gt, current_indices),
                                _mm_andnot_ps(cmp_gt, max_indices));
        max_values = _mm_max_ps(mag_sq, max_values);

        current_indices = _mm_add_ps(current_indices, indices_increment);
    }

    // Reduce
    __VOLK_ATTR_ALIGNED(16) float max_values_buffer[4];
    __VOLK_ATTR_ALIGNED(16) float max_indices_buffer[4];
    _mm_store_ps(max_values_buffer, max_values);
    _mm_store_ps(max_indices_buffer, max_indices);
    ...
    for (unsigned i = 0; i < 4; i++) {
        if (max_values_buffer[i] > max) {
            ...
            index = (uint32_t)max_indices_buffer[i];
        }
    }

    // Handle tail
    const unsigned tail_start_sse = num_points & (~3u);
    if (tail_start_sse < num_points) {
        uint16_t tail_index = 0;
        volk_32fc_index_max_16u_generic(
            &tail_index, src0, num_points - tail_start_sse);
        const float re = lv_creal(src0[tail_index]);
        const float im = lv_cimag(src0[tail_index]);
        const float tail_max = re * re + im * im;
        if (tail_max > max) {
            index = tail_start_sse + tail_index;
        }
    }
...
static inline void volk_32fc_index_max_16u_u_sse3(uint16_t* target,
                                                   const lv_32fc_t* src0,
                                                   uint32_t num_points)
{
    num_points = (num_points > USHRT_MAX) ? USHRT_MAX : num_points;
    ...
    for (; i < bound; ++i) {
        xmm1 = _mm_loadu_ps((const float*)src0);
        xmm2 = _mm_loadu_ps((const float*)&src0[2]);
    ...
    if (num_bytes >> 4 & 1) {
        xmm2 = _mm_loadu_ps((const float*)src0);
...
static inline void volk_32fc_index_max_16u_u_avx(uint16_t* target,
                                                   const lv_32fc_t* src0,
                                                   uint32_t num_points)
{
    num_points = (num_points > USHRT_MAX) ? USHRT_MAX : num_points;

    const __m256 indices_increment = _mm256_set1_ps(8.0f);
    /*
     * Index order matches the lane-crossing artifact of _mm256_shuffle_ps:
     * positions map to complex indices [0,1,4,5,2,3,6,7].
     */
    __m256 current_indices = _mm256_setr_ps(0, 1, 4, 5, 2, 3, 6, 7);

    __m256 max_values = _mm256_setzero_ps();
    __m256 max_indices = _mm256_setzero_ps();
    ...
    for (unsigned i = 0; i < num_points / 8u; ++i) {
        ...
        __m256 cmp_gt = _mm256_cmp_ps(mag_sq, max_values, _CMP_GT_OS);
        max_indices = _mm256_blendv_ps(max_indices, current_indices, cmp_gt);
        max_values = _mm256_max_ps(mag_sq, max_values);

        current_indices = _mm256_add_ps(current_indices, indices_increment);
    }

    // Reduce
    __VOLK_ATTR_ALIGNED(32) float max_values_buffer[8];
    __VOLK_ATTR_ALIGNED(32) float max_indices_buffer[8];
    _mm256_store_ps(max_values_buffer, max_values);
    _mm256_store_ps(max_indices_buffer, max_indices);
    ...
    // Handle tail
    const unsigned tail_start_avx = num_points & (~7u);
    if (tail_start_avx < num_points) {
        uint16_t tail_index = 0;
        volk_32fc_index_max_16u_generic(
            &tail_index, src0, num_points - tail_start_avx);
        const float re = lv_creal(src0[tail_index]);
        const float im = lv_cimag(src0[tail_index]);
        const float tail_max = re * re + im * im;
        if (tail_max > max) {
            index = tail_start_avx + tail_index;
        }
    }
...
static inline void volk_32fc_index_max_16u_a_sse(uint16_t* target,
                                                  const lv_32fc_t* src0,
                                                  uint32_t num_points)
{
    num_points = (num_points > USHRT_MAX) ? USHRT_MAX : num_points;
    ...
    for (unsigned i = 0; i < num_points / 4u; ++i) {
        __m128 in0 = _mm_load_ps((const float*)src0);
        __m128 in1 = _mm_load_ps((const float*)(src0 + 2));
        src0 += 4;
    ...
    // Handle tail
    const unsigned a_tail_start_sse = num_points & (~3u);
    if (a_tail_start_sse < num_points) {
        uint16_t tail_index = 0;
        volk_32fc_index_max_16u_generic(
            &tail_index, src0, num_points - a_tail_start_sse);
        const float re = lv_creal(src0[tail_index]);
        const float im = lv_cimag(src0[tail_index]);
        const float tail_max = re * re + im * im;
        if (tail_max > max) {
            index = a_tail_start_sse + tail_index;
        }
    }
...
static inline void volk_32fc_index_max_16u_a_avx(uint16_t* target,
                                                   const lv_32fc_t* src0,
                                                   uint32_t num_points)
{
    num_points = (num_points > USHRT_MAX) ? USHRT_MAX : num_points;
    ...
    // Handle tail
    const unsigned a_tail_start_avx = num_points & (~7u);
    if (a_tail_start_avx < num_points) {
        uint16_t tail_index = 0;
        volk_32fc_index_max_16u_generic(
            &tail_index, src0, num_points - a_tail_start_avx);
        const float re = lv_creal(src0[tail_index]);
        const float im = lv_cimag(src0[tail_index]);
        const float tail_max = re * re + im * im;
        if (tail_max > max) {
            index = a_tail_start_avx + tail_index;
        }
    }
