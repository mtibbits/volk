 ...
 * \b Overview
 *
 * Multiplies a complex vector by the conjugate of a second complex vector and
 * returns the complex result: c[i] = a[i] * conj(b[i]). When applied to the
 * same signal (a = b), this computes the squared magnitude of each sample.
 *
 * Conjugate multiplication is a fundamental operation in many DSP algorithms,
 * including cross-correlation for synchronization and timing recovery, frequency
 * offset estimation between successive symbols, and Doppler shift correction.
 * It is also the core step in computing power spectral density.
 *
 ...
 *
 * \b Inputs
 * \li aVector: The first input vector of complex floats (lv_32fc_t).
 * \li bVector: The second input vector of complex floats (lv_32fc_t) that is conjugated.
 * \li num_points: The number of complex samples.
 *
 * \b Outputs
 * \li cVector: The output vector of complex floats (lv_32fc_t).
 *
 * \b Example
 * Compute magnitude squared using x * conj(x): (3+4j) * conj(3+4j) = 25+0j.
 * \code
 * unsigned int N = 4;
 ...
 * }
 *
 * // Expected: (3+4j) * conj(3+4j) = (3+4j)(3-4j) = 9+16 = 25+0j
 * lv_32fc_t expected = lv_cmake(25.0f, 0.0f);
 *
 * volk_32fc_x2_multiply_conjugate_32fc(out, a, a, N);
 *
...
static inline void volk_32fc_x2_multiply_conjugate_32fc_u_sse(lv_32fc_t* cVector,
                                                               const lv_32fc_t* aVector,
                                                               const lv_32fc_t* bVector,
                                                               unsigned int num_points)
{
    ...
    for (; number < halfPoints; number++) {
        ...
        __m128 z = _mm_add_ps(_mm_mul_ps(x, dreal), _mm_mul_ps(nswap, dimagconj));
...
static inline void volk_32fc_x2_multiply_conjugate_32fc_u_avx512f(lv_32fc_t* cVector,
                                                                    const lv_32fc_t* aVector,
                                                                    const lv_32fc_t* bVector,
                                                                    unsigned int num_points)
{
    ...
    for (; number < eighthPoints; number++) {
        ...
        __m512 z = _mm512_complexconjugatemul_ps(x, y);
...
static inline void volk_32fc_x2_multiply_conjugate_32fc_a_sse(lv_32fc_t* cVector,
                                                               const lv_32fc_t* aVector,
                                                               const lv_32fc_t* bVector,
                                                               unsigned int num_points)
{
...
static inline void volk_32fc_x2_multiply_conjugate_32fc_a_avx512f(lv_32fc_t* cVector,
                                                                    const lv_32fc_t* aVector,
                                                                    const lv_32fc_t* bVector,
                                                                    unsigned int num_points)
{
