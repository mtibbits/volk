...
volk_32f_cos_32f_u_avx(float* bVector, const float* aVector, unsigned int num_points)
{
    ...
    for (; number < eighthPoints; number++) {
        ...
        // Convert n to integer — split into 128-bit halves for AVX-only integer ops
        __m128i n_lo = _mm_cvtps_epi32(_mm256_castps256_ps128(n_f));
        __m128i n_hi = _mm_cvtps_epi32(_mm256_extractf128_ps(n_f, 1));

        // Reconstruct cos(x) based on quadrant (n mod 4):
        // n&1 == 0: use cos_r, n&1 == 1: use sin_r
        // (n+1)&2 == 0: positive, (n+1)&2 != 0: negative
        __m128i n_and_1_lo = _mm_and_si128(n_lo, ones);
        __m128i n_and_1_hi = _mm_and_si128(n_hi, ones);
        __m128i n_plus_1_and_2_lo = _mm_and_si128(_mm_add_epi32(n_lo, ones), twos);
        __m128i n_plus_1_and_2_hi = _mm_and_si128(_mm_add_epi32(n_hi, ones), twos);

        // swap_mask: where n&1 != 0, we use sin instead of cos
        __m128 swap_lo = _mm_castsi128_ps(_mm_cmpeq_epi32(n_and_1_lo, ones));
        __m128 swap_hi = _mm_castsi128_ps(_mm_cmpeq_epi32(n_and_1_hi, ones));
        __m256 swap_mask =
            _mm256_insertf128_ps(_mm256_castps128_ps256(swap_lo), swap_hi, 1);
        __m256 result = _mm256_blendv_ps(cos_r, sin_r, swap_mask);

        // neg_mask: where (n+1)&2 != 0, we negate the result
        __m128 neg_lo = _mm_castsi128_ps(_mm_cmpeq_epi32(n_plus_1_and_2_lo, twos));
        __m128 neg_hi = _mm_castsi128_ps(_mm_cmpeq_epi32(n_plus_1_and_2_hi, twos));
        __m256 neg_mask =
            _mm256_insertf128_ps(_mm256_castps128_ps256(neg_lo), neg_hi, 1);
    ...
    volk_32f_cos_32f_generic(bPtr, aPtr, num_points - number);
}
...
static inline void
volk_32f_cos_32f_u_avx_fma(float* bVector, const float* aVector, unsigned int num_points)
{
    float* bPtr = bVector;
    const float* aPtr = aVector;
...
static inline void volk_32f_cos_32f_u_avx512dq(float* cosVector,
                                               const float* inVector,
                                               unsigned int num_points)
{
    float* cosPtr = cosVector;
    ...
    for (; number < sixteenPoints; number++) {
        ...
        // neg_mask: where (n+1)&2 != 0, we negate the result (AVX512DQ: float xor)
        __mmask16 neg_mask = _mm512_cmpeq_epi32_mask(n_plus_1_and_2, twos);
        result = _mm512_mask_xor_ps(result, neg_mask, result, sign_bit);
...
static inline void
volk_32f_cos_32f_a_avx_fma(float* bVector, const float* aVector, unsigned int num_points)
{
    float* bPtr = bVector;
    const float* aPtr = aVector;
...
static inline void volk_32f_cos_32f_a_avx512dq(float* cosVector,
                                               const float* inVector,
                                               unsigned int num_points)
{
    float* cosPtr = cosVector;
